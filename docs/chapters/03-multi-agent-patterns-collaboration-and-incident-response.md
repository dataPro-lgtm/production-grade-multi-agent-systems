...
description: 从架构模式、委托合同、状态所有权与协议分层，到生产事故响应系统
---

# 第 03 章：让多个 Agent 可靠协作——架构模式、协作合同与事件响应

凌晨 10 点 02 分，支付成功率突然从 99.6% 跌到 93.1%。告警平台在一分钟内同时抛出三类异常：
...
本章将沿着这起支付事故，建立一套从模式选择到生产落地的完整方法。我们会讨论不同协作模式解决什么问题，怎样用合同约束委托和结果，怎样划分状态所有权，以及 A2A、MCP 与工作流引擎分别位于哪一层。最后，再把这些判断收敛为一个可审计的事故响应系统。
