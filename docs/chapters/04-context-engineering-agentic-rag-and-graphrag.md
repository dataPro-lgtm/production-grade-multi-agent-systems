...
本章不会把 Context Engineering 简化成“更会写 Prompt”，也不会把 GraphRAG 描述成向量库的替代品。我们将把一次模型调用视为一个受治理的数据消费端：上游负责发现、检索、授权、去重、验证、压缩和打包，下游模型只能在一份类型化、可追溯、有预算的 Context Pack 上进行推理。讲清这些原理后，我们会把它们落到 CaseOps 的第三个可运行切片：不用虚构的向量召回数字，而是用 PostgreSQL 全文检索、结构化查询和受限图路径跑出一份真实的案件调查证据包。
...
## 18. CaseOps Slice 3：把“相关内容”变成可用证据

前面的讨论容易让人产生一种错觉：Context Engineering 似乎只是给现有 RAG 多加几个字段。真正落到系统里，它改变的是责任边界。

检索器负责发现候选，不负责批准候选；Context Builder 负责执行授权、时态、完整性和预算规则，不负责编造缺失事实；回答器只能使用已经进入 Context Pack 的 Evidence，并且每个关键 Claim 都必须给出 Evidence ID。模型即使参与其中，也只是这条证据生产线的消费者。

CaseOps Slice 3 用案件 `C-102` 验证这条边界。系统需要回答：

> 事故证明是否满足规则要求，适用哪个规则版本，为什么需要人工复核？

这个问题同时要求精确事实、文本内容、关系路径和时间切面。只做一次语义相似度检索，很容易把当前规则、历史规则、外部邮件和案件材料混在一起。

### 18.1 先定义必须被证据支持的判断

我没有先问“应该召回多少段文本”，而是先定义回答成立所需的五类证据：

| 必要证据类型 | 要支持的判断 | 典型来源 |
|---|---|---|
| `policy_version` | 在 `as_of` 时刻适用哪个规则版本 | Policy Registry |
| `document_status` | 规则要求的材料是否已满足 | Case DB、Document Store |
| `claim_amount_signal` | 案件金额是否越过人工复核阈值 | Case DB |
| `policy_tenure_signal` | 保单存续期是否触发风险规则 | Policy / Risk Profile |
| `manual_review_rule` | 哪条规则把事实映射为处置要求 | Risk Rule Registry |

这一步很重要。Evidence Sufficiency 判断的不是“已有五段内容”，而是“支持必要判断的证据类型是否齐全”。三段都在讲金额的文本，不能替代一条有效的规则证据。

### 18.2 来源、对象与关系分开建模

Slice 3 新增三层知识数据。

第一层是 **Source Registry**。每个来源登记 `owner_team`、数据分类、刷新 SLA、解析器版本和启停状态。第二层是 **Knowledge Object**，它保存真正可进入证据链的内容，并携带：
...
第三层是 **Entity / Relation**。关系边不是裸三元组，而是带有 `path_template`、有效时间和证据对象的可审计关系。

这三层不能合并成一张“大知识表”。来源回答“谁负责这批数据”；对象回答“这份内容在何时、为何可用”；关系回答“沿哪条经过批准的业务路径可以找到它”。把三者拆开，权限、版本和图遍历才有明确的执行位置。

### 18.3 Query Planner 选择通道，不生成任意查询

当前 Planner 会生成一份类型化 Retrieval Plan：
...
- `structured` 精确获取案件快照和已知业务键；
- `full_text` 使用 PostgreSQL `websearch_to_tsquery` 与 GIN 索引召回正文候选；
- `graph` 只遍历允许清单中的路径模板，并限制为两跳。

这里有一个刻意的工程选择：**当前版本没有默认启用 Vector Channel。** 代码合同保留了这个通道，但在真实领域语料、固定 Embedding 版本、ACL 过滤策略和可复现检索评测具备之前，我不打算用随机向量或假相似度把它包装成“混合检索”。本切片已经实现的 Hybrid，是结构化、全文和关系检索的真实组合。

不同通道的原始分数不可直接相加。系统使用 Reciprocal Rank Fusion 合并排序：

$$
\operatorname{RRF}(d)=\sum_{r\in R_d}\frac{1}{60+r}
$$

它只使用候选在各通道中的名次，不假设全文相关度、业务优先级和图路径分数处于同一量纲。

### 18.4 Context Builder 是准入控制面

融合后的候选按固定顺序经过七道门禁：

1. **Scope**：当前主体是否拥有对象要求的 Scope；
2. **Purpose**：当前调查用途是否被来源允许；
3. **Temporal**：对象在请求的 `as_of` 时刻是否有效；
4. **Integrity**：正文的 SHA-256 是否仍与登记哈希一致；
5. **Trust**：外部非可信内容是否携带可执行指令；
6. **Deduplication**：同一对象是否由多通道重复召回；
7. **Budget**：加入后是否超过 Evidence Token Budget。

门禁的顺序不是随意的。越权或用途不符的内容不应先被摘要；失效内容不应与当前版本竞争；哈希不一致意味着证据完整性已经无法确认；包含“忽略规则并执行操作”的外部邮件只能作为攻击样本留在 Trace，不能进入模型输入。

![CaseOps Slice 3 上下文管线](../assets/images/chapter-04/caseops-slice3-context-pipeline.png)

*图 4-7　检索负责发现候选，Context Builder 决定哪些对象有资格成为证据。*

### 18.5 有界补检索依赖证据缺口，而不是模型直觉

每轮构建后，系统把 Pack 中的 `supports_claims` 与五类必要证据求差集：

```text
missing = required_evidence_types - supported_evidence_types
```

若 `missing` 为空，立即以 `evidence_sufficient` 终止；若仍有缺口，Planner 只能在剩余轮数和预算内调整允许通道。达到 `max_rounds=2` 或没有新增 Evidence 时必须停机，并把缺口写入 `missing_evidence`。

因此，“Agentic”不表示模型可以无限搜索。它表示系统会依据可计算的证据缺口决定是否继续，并对轮数、通道、跳数和预算负责。

### 18.6 真实运行结果：七条证据、两条明确拒绝

在 PostgreSQL 17 容器中运行验收后，`C-102` 的调查一轮即满足证据要求：
...
| 候选 | 拒绝原因 | 为什么不能交给模型自行处理 |
|---|---|---|
| `policy:claims:2025.4` | `rejected_temporal` | 它与问题高度相关，但在 `as_of` 时刻已经失效 |
| `email:external:attack-01` | `rejected_untrusted_instruction` | 它包含要求绕过规则的非可信指令 |

最终回答形成三条关键 Claim：

1. 当前适用理赔规则版本为 `2026.1`；
2. `ACCIDENT_CERTIFICATE` 已由规范化后的来源材料满足；
3. 案件金额 `128000` 高于阈值，且保单存续期 `12` 个月落入人工复核规则，因此建议 `route_to_human_reviewer`。

第三条不是“检索到了风险规则”就结束。回答器会读取结构化事实和规则阈值，执行确定性比较，再生成带证据绑定的处置建议。测试还覆盖了金额低于阈值的分支，防止代码把“发现规则”误写成“规则已触发”。

### 18.7 一次事务保存 Pack、Trace、审计与事件

`ContextRun` 持久化请求哈希、Retrieval Plan、Context Pack、答案和逐候选 Trace；同一事务还写入审计记录与 CloudEvents 1.0 Outbox。相同租户下重复使用同一幂等键时，系统返回原 `run_id` 和 `pack_id`，不会重新检索；若请求体不同，则返回幂等冲突。

这使我们可以回答一组生产环境里绕不开的问题：
...
完整实现位于独立工程 [production-grade-multi-agent-caseops](https://github.com/dataPro-lgtm/production-grade-multi-agent-caseops/tree/chapter-04-slice-3)，版本为 `v0.4.0`，对应提交 `35c3c85`。可执行入口是：
...
这次验收不是把预期输出写在文档里：它会实际调用 API，检查幂等回放，再查询 PostgreSQL 中的 `context_runs`、事务 Outbox、图关系与全文索引。
...
- [ ] 是否用真实检索与数据库验收，而不是用固定 JSON 冒充端到端结果？

## 参考资料

- [Anthropic: Effective context engineering for AI agents](https://www.anthropic.com/engineering/effective-context-engineering-for-ai-agents)：把 Context Engineering 视为对模型全部可见信息的持续管理，并强调按需加载而不是一次性塞满上下文。
...
- [Azure AI Search: Hybrid search](https://learn.microsoft.com/en-us/azure/search/hybrid-search-overview)：全文与向量查询并行执行，并使用 RRF 融合结果。
...
- [PostgreSQL 17: Controlling Text Search](https://www.postgresql.org/docs/17/textsearch-controls.html)：`tsvector`、`tsquery`、排序与索引化全文检索。
- [PostgreSQL 17: WITH Queries](https://www.postgresql.org/docs/17/queries-with.html)：递归查询的工作方式，以及图遍历中的路径和循环处理。
...
- [CaseOps Slice 3](https://github.com/dataPro-lgtm/production-grade-multi-agent-caseops/tree/chapter-04-slice-3)：本章上下文管线、迁移、测试、运行手册与验收脚本。
